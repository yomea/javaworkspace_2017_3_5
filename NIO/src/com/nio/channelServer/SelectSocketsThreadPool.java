...
public class SelectSocketsThreadPool extends SelectSockets {
	 ...
	 * method is invoked from(被调用) the go( ) method in the parent class. This
	 * handler delegates（委托） to a worker thread in a thread pool to service the
	 * channel, then returns immediately.
	 *
	 * @param key
	 *
	...
	/**
	 * A worker thread class which can drain（排空） channels and echo-back（回显） the
	 * input. Each instance is constructed with a reference（参考） to the owning
	 * thread pool object. When started, the thread loops forever waiting to be
	 * awakened to service the channel associated with a SelectionKey object.
	 * The worker is tasked by calling its serviceChannel( ) method with a
